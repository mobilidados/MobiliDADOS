...
PNT <- function(i, j) {
  ...
  TMA_buf <- read_rds(paste0('./codigos/tt_isochrones-master/output/pnt/', i, '_buffer_', j, '.rds'))   #buffer com distancia real
  TMA_buf <- st_transform(TMA_buf, 4326)
  TMA_buf <- st_transform(TMA_buf, subset(munis_df, rm == i)$espg)
  ...
  write.xlsx(Resultados_pnt_final, 
             paste0('./resultados/pnt/', j, '/network/', i, '_resultados_pnt.xlsx'),
...
PNT('rmsp', 2019) #se desejar calcular para outro ano, basta trocar
...
#criar lista
files <- list.files(path = './resultados/pnt/2019/network',
...
#salvar
write.xlsx(juntos, './resultados/pnt/2019/network/consolidado/resultados_rm.xlsx')

#4.2 criar tabela para database
# abrir tabela
todos <- read.xlsx('./resultados/pnt/2019/network/consolidado/resultados_rm.xlsx')
...
write.xlsx(todos_long, './resultados/pnt/2019/network/consolidado/resultados_rm_database.xlsx') # salvar
