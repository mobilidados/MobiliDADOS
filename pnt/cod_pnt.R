...
estacoes <- st_read('./dados/tma/estacoes_2018.shp') #insere o nome do arquivo no qual salvou as estacoes para abri-lo
...
dados <- read_rds('./dados/dados_ibge/dados_setores.rds') %>% mutate(Cod_setor=as.character(Cod_setor))
...
juntar_setores_dados <- function (rm) {
  setores <- read_rds(paste0('./dados/setores/setores_', rm, '.rds')) %>% st_sf() #inserir o nome do arquivo no qual salvou os setores para abri-lo
...
setores_dados <- setores_dados %>% st_set_precision(1000000) %>% lwgeom::st_make_valid() #corrige shapes que podem possuir algum defeito de feicao
...
PNT_ano <- function(i) {
  ...
  message(paste0('Olá, ', unique(munis_df$rm),'! =)', "\n"))
  ...
  message(paste0('Estacoes filtradas e buffer criado.',"\n"))
  
  setores_dados <- st_transform(setores_dados, 3857) #transforma projecao
  ...
  message(paste0('E o resultado do PNT é...',"\n"))
  
  print(Resultados_rm) #se desejar verficar a tabela, rode esta linha de codigo
  
  write.xlsx(Resultados_rm, paste0('./resultados/resultados_pnt_', unique(munis_df$rm),'_', i, '_.xlsx')) #salva os resultados finais na pasta com o nome da RM  
}
