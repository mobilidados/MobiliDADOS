...
PNT <- function(i, j) {
  ...
  setores <- read_rds('./apoio/setores_rms.rds') %>% 
    filter(code_muni %in% base_rms2$COD_MUN[base_rms2$rm == i]) %>%
    mutate(Ar_m2 = unclass(st_area(.)), Cod_setor = as.character(Cod_setor))
  setores <- st_transform(setores, 4326) #transforma projecao
  setores <- st_transform(setores, subset(munis_df_rms, rms == i)$espg) #transforma projecao
  ...
  setores_dados <- setores_dados %>% st_set_precision(1000000) %>% 
    ...
    st_transform(., 4326)
  setores_dados <- st_transform(setores_dados, subset(munis_df_rms, rms == i)$espg) #transforma projecao
  
  message(paste0('abrindo buffer ...',"\n"))
  
  TMA_buf <- read_rds(paste0('./output/', j,'/buffer/rds/rms/', i, '_buffer_',j, '.rds'))   #buffer com distancia real
  TMA_buf <- st_transform(TMA_buf, 4326)
  TMA_buf <- st_transform(TMA_buf, subset(munis_df_rms, rms == i)$espg)
  ...
  write.xlsx(Resultados_pnt_final, 
             paste0('./resultados/', j, '/rms/', i, '_resultados_pnt.xlsx'),
  ...
  st_write(select(setores_entorno,
                  ...
                  Pessoa_0_1, Pessoa_0_meio, Pessoa_meio_1, Pessoa_1_3, Pessoa_3_mais, 
                  Negros, brancas),  
           paste0('./output/', j, '/entorno/rms/shp/', i, '_entorno.shp'),append = TRUE)
  write_rds(setores_entorno,  
            paste0('./output/', j, '/entorno/rms/rds/', i, '_entorno.rds'))
...
pbmapply(safely(PNT),munis_df_rms$rms %>% unique(),2021)

pbmapply(safely(PNT),munis_df_rms$rms %>% unique(),2019)
pbmapply(safely(PNT),munis_df_rms$rms %>% unique(),2018)
Sys.sleep(2)
pbmapply(safely(PNT),munis_df_rms$rms %>% unique(),2017)
Sys.sleep(2)
pbmapply(safely(PNT),munis_df_rms$rms %>% unique(),2016)
Sys.sleep(2)
pbmapply(safely(PNT),munis_df_rms$rms %>% unique(),2015)
Sys.sleep(2)
pbmapply(safely(PNT),munis_df_rms$rms %>% unique(),2014)
Sys.sleep(2)
pbmapply(safely(PNT),munis_df_rms$rms %>% unique(),2013)
Sys.sleep(2)
pbmapply(safely(PNT),munis_df_rms$rms %>% unique(),2012)
Sys.sleep(2)
pbmapply(safely(PNT),munis_df_rms$rms %>% unique(),2011)
Sys.sleep(2)
pbmapply(safely(PNT),munis_df_rms$rms %>% unique(),2010)
...
files <- list.files(path = "D:/Projetos/mobilidados/pnt_verificação/resultados/2021/rms",
...
rms_dados_upload  <- pnt_format_1 %>% mutate(territorio = factor(territorio, levels = ordem)) %>%
  arrange(territorio)  



rms_pnt_db <- juntos[c(1,2,3,5)] %>% 
...
rms_pnt_resultado <- juntos[c(5,4,1)] %>% subset(indicador == "Pop") %>% 
  mutate(territorio = factor(territorio, levels = ordem),
         indicador = factor(indicador, levels = ordem3),
         ano = 2021) %>% arrange(territorio, indicador) %>% select(territorio, resultado, ano) 

write.xlsx(rms_dados_upload, "D:/Projetos/mobilidados/pnt_verificação/resultados/2021/0_consolidado/rms_dados_upload_2021.xlsx") # salvar
write.xlsx(rms_pnt_db, "D:/Projetos/mobilidados/pnt_verificação/resultados/2021/0_consolidado/rms_pnt_db_2021.xlsx") # salvar
write.xlsx(rms_pnt_db, "D:/Projetos/mobilidados/pnt_verificação/resultados/0_geral/capitais/rms_pnt_db_2021.xlsx") # salvar
write.xlsx(rms_pnt_resultado, "D:/Projetos/mobilidados/pnt_verificação/resultados/2021/0_consolidado/rms_pnt_resultado_2021.xlsx")
write.xlsx(rms_pnt_resultado, "D:/Projetos/mobilidados/pnt_verificação/resultados/0_geral/capitais//rms_pnt_resultado_2021.xlsx")# salvar
...
for(i in 2010:2021){
  files <- list.files(path = paste0("D:/Projetos/mobilidados/pnt_verificação/resultados/",i,"/rms"),
                      pattern = "\\.xlsx$", full.names = TRUE)
  ...
  rms_pnt_resultado2 <- juntos[c(5,4,1)] %>% subset(indicador %in% ordem3) %>% 
    mutate(territorio = factor(territorio, levels = ordem),
           indicador = factor(indicador, levels = ordem3),
           ano = i) %>% arrange(territorio, indicador) %>% select(territorio, resultado, indicador, ano) 
  
  write.xlsx(rms_dados_upload, paste0("D:/Projetos/mobilidados/pnt_verificação/resultados/",i,"/0_consolidado/rms_dados_upload_",i,".xlsx")) # salvar
  write.xlsx(rms_pnt_db, paste0("D:/Projetos/mobilidados/pnt_verificação/resultados/",i,"/0_consolidado/rms_pnt_db_",i,".xlsx")) # salvar
  write.xlsx(rms_pnt_db, paste0("D:/Projetos/mobilidados/pnt_verificação/resultados/0_geral/rms/rms_pnt_db_",i,".xlsx")) # salvar
  write.xlsx(rms_pnt_resultado, paste0("D:/Projetos/mobilidados/pnt_verificação/resultados/",i,"/0_consolidado/rms_pnt_resultado_",i,".xlsx"))
  write.xlsx(rms_pnt_resultado, paste0("D:/Projetos/mobilidados/pnt_verificação/resultados/0_geral/rms//rms_pnt_resultado_",i,".xlsx"))# salvar
  write.xlsx(rms_pnt_resultado2, paste0("D:/Projetos/mobilidados/pnt_verificação/resultados/0_geral/rms//rms_pnt_tipo",i,".xlsx"))
}


files <- list.files(path = "D:/Projetos/mobilidados/pnt_verificação/resultados/0_geral/rms",
                    pattern = "rms_pnt_db", full.names = TRUE)

files2 <- list.files(path = "D:/Projetos/mobilidados/pnt_verificação/resultados/0_geral/rms",
                    pattern = "rms_pnt_resultado", full.names = TRUE)

files3 <- list.files(path = "D:/Projetos/mobilidados/pnt_verificação/resultados/0_geral/rms",
                     pattern = "rms_pnt_tipo", full.names = TRUE)
...
write.xlsx(tab_final_1, paste0("D:/Projetos/mobilidados/pnt_verificação/resultados/0_geral/rms/tab_final_1.xlsx"))
write.xlsx(tab_final_2, paste0("D:/Projetos/mobilidados/pnt_verificação/resultados/0_geral/rms/tab_final_2.xlsx"))
write.xlsx(tab_final_3, paste0("D:/Projetos/mobilidados/pnt_verificação/resultados/0_geral/rms/tab_final_3.xlsx"))
