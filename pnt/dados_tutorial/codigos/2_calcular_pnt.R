...
# Defnir diretorio
setwd('C:/Users/berna/Desktop/tutorial_pnt') #altere o caminho para a pasta onde deseja salvar os arquivos
...
dados <- read_rds('./dados/dados_setores.rds') %>% 
...
juntar_setores_dados <- function (rm) {
  setores <- read_rds(paste0('./dados/setores_', rm, '.rds')) %>% st_sf() #inserir o nome do arquivo no qual salvou os setores para abri-lo
...
PNT_ano <- function(i) {
  ...
  TMA_buf <- read_rds(paste0('./dados/', unique(munis_df$rm), '_buffer_', i, '.rds'))   #buffer com distancia real
  TMA_buf <- st_transform(TMA_buf, 4326)
  
  setores_dados <- setores_dados %>% st_set_precision(1000000) %>% st_make_valid() #corrigir shapes que podem possuir algum defeito de feicao
