...
mapa_pnb <- function(i){
  ...
  PNB <- ggplot()+
    ...
    geom_sf(data=hex_nn, aes(fill = dens), colour = NA, alpha = 0.8) +
    ...
    geom_sf(data = infra_ciclo_muni, colour = "darkturquoise", size = 0.2)+ # mudar espessura da linha
    geom_sf(data=muni, fill=NA, colour = "black", alpha = 0.4)+
    geom_sf(data = infra_ciclo_buf, fill="gray85", colour = "black", size = 0.05, alpha = 0.2)+  
    # mudar espessura da linha
    ...
    coord_sf(expand = F, xlim = c(st_bbox(muni)[[1]]-0.05, st_bbox(muni)[[3]]+0.05),
             ylim = c(st_bbox(muni)[[2]]-0.03, st_bbox(muni)[[4]]+0.03))  #Zoom
  
  message(paste0('mapa de ', i," criado"))
  
  #salvar
  ggsave(PNB, 
         file= paste0('./outros/mapas/PNB/hex/', i, '.png'), 
