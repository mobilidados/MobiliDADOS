...
filtrar_infra_ciclo <- function(cid, ano){
  ...
  infra_ciclo <- st_read(paste0('./pnb/infra_ciclo/',ano,'/infra_ciclo_', 
  ...
  infra_ciclo_pnpb <- infra_ciclo[(infra_ciclo$tipologia %in% 
                                    c('Ciclovia')),]#filtra infraestrutura cicloviaria para considerar apenas ciclovias
  
  infra_ciclo_pnb <- st_transform(infra_ciclo_pnb, 4326)#transforma projecao
  
  infra_ciclo_pnbp <- st_transform(infra_ciclo_pnpb, 4326)#transforma projecao
  ...
  st_write(infra_ciclo_pnb, paste0('./pnb/dados/infra_cicloviaria/pnb/geofabrik_filtrado/',ano,'/shp/', 
                                subset(munis_df, code_muni==cid)$name_muni,
                                '_filtrado.shp'), append = FALSE)
  write_rds(infra_ciclo_pnb, paste0('./pnb/dados/infra_cicloviaria/pnb/geofabrik_filtrado/',ano,'/rds/',
                                   subset(munis_df, code_muni==cid)$name_muni,
                                   '_filtrado.rds'))
  st_write(infra_ciclo_pnpb, paste0('./pnb/dados/infra_cicloviaria/pnpb/geofabrik_filtrado/',ano,'/shp/', 
                                   subset(munis_df, code_muni==cid)$name_muni,
                                   '_filtrado.shp'), append = FALSE)
  write_rds(infra_ciclo_pnpb, paste0('./pnb/dados/infra_cicloviaria/pnpb/geofabrik_filtrado/',ano,'/rds/',
                                    subset(munis_df, code_muni==cid)$name_muni,
                                    '_filtrado.rds'))
}
...
lista_arquivos_pnb <- list.files(paste0("./pnb/dados/infra_cicloviaria/pnb/geofabrik_filtrado/",ano,"/rds"), pattern='*\\.rds', full.names = TRUE)
lista_arquivos_pnpb <- list.files(paste0("./pnb/dados/infra_cicloviaria/pnpb/geofabrik_filtrado/",ano,"/rds"), pattern='*\\.rds', full.names = TRUE)

caps_lines_pnb <- do.call("rbind", lapply(lista_arquivos_pnb, read_rds))
caps_lines_pnpb <- do.call("rbind", lapply(lista_arquivos_pnpb, read_rds))

# Exporta em dois formatos diferentes por segurança

st_write(caps_lines_pnb, paste0("./pnb/dados/infra_cicloviaria/pnb/geofabrik_filtrado/",ano,"/shp","/geofabrik_capitais_filtrado.shp"), append = FALSE)
write_rds(caps_lines_pnb, paste0("./pnb/dados/infra_cicloviaria/pnb/geofabrik_filtrado/",ano,"/rds","/geofabrik_capitais_filtrado.rds"))


st_write(caps_lines_pnpb, paste0("./pnb/dados/infra_cicloviaria/pnpb/geofabrik_filtrado/",ano,"/shp","/geofabrik_capitais_filtrado.shp"), append = FALSE)
write_rds(caps_lines_pnpb, paste0("./pnb/dados/infra_cicloviaria/pnpb/geofabrik_filtrado/",ano,"/rds","/geofabrik_capitais_filtrado.rds"))
