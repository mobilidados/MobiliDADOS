...
# É importante tomar cuidado, pois o pacote saiu do CRAN.
# Caso exita outro pacote semelhante é preferível utilzar

# Esse pacote faz scrap dessa págnina:
# http://tabnet.datasus.gov.br/cgi/deftohtm.exe?sim/cnv/obt10br.def
# Caso exista alguma dúvida ir direto ao site conferir
...
# Seleciona a estação de trabalho
...
# Função para baixar os dados referentes ao número de obitos por Grupo Cid
...
pegar_dados_mun <- function(i){
  ...
  assign(subset(lista_obt, grupo == i)$var, # assinala o nome do objeto.
         sim_obt10_mun(linha = "Município", # Dados por municipio.
                       coluna = "Ano do Óbito", # Dados por ano de óbito.
                       conteudo = 2, # Escolha por local de ocorrência.
                       periodo = periodo,
                       cor_raca = c("Preta","Parda" ),# Período desejado. Bom verificar qual é o útimo ano disponível
                       grupo_cid10 = i) %>% print(.,row.names = FALSE), envir = .GlobalEnv)  #grupo cid10 desejado 
}
...
ativo <- sim_obt10_mun(linha = "Município", # Dados por municipio.
                       coluna = "Ano do Óbito", # Dados por ano de óbito.
                       conteudo = 2, # Escolha por local de ocorrência.
                       periodo = periodo, # Período desejado. Bom verificar qual é o útimo ano disponível
                       grupo_cid10 = c("Pedestre traumatizado em um acidente de transporte",
                                       "Ciclista traumatizado em um acidente de transporte"),
                       cor_raca = c("Preta","Parda" )) %>% print(.,row.names = FALSE)
...
total_outros <- sim_obt10_mun(linha = "Município", # Dados por municipio.
                              ...
                              conteudo = 2, # Escolha por local de ocorrência.
                              periodo = periodo, # Período desejado. Bom verificar qual é o útimo ano disponível
                              grupo_cid10 = c("Ocupante ônibus traumat acidente de transporte",
                                              "Ocupante triciclo motorizado traumat acid transp",
                                              "Ocupante caminhonete traumat acidente transporte",
                                              "Ocupante veíc transp pesado traumat acid transp",
                                              "Outros acidentes de transporte terrestre"),
...
# Seprando os Códigos dos Municípios e retirar linha de total

for(i in lista2){
  
  assign(i, get(i) %>% mutate(code_muni6 = substr(Município, 1, 6), Município = NULL, Total = NULL, tipo = i) %>% .[-1,],
         envir = .GlobalEnv)
}
...
basao2 <- basao2 %>% mutate(tipo = case_when(tipo == lista3[1] ~ "Total",
                                             ...
                                             tipo == lista3[5] ~ "Ocupantes de automóvel",
...
write.csv2(basao, "F:/Projetos/mobilidados/datasus_verificacao/output/taxa_mort_mun_negres.csv", row.names = FALSE)
write.csv2(basao2, "F:/Projetos/mobilidados/datasus_verificacao/output/num_mort_mun_negres.csv", row.names = FALSE)
write.csv2(taxa_final_rm, "F:/Projetos/mobilidados/datasus_verificacao/output/taxa_final_rm_negres.csv", row.names = FALSE)
write.csv2(tab_morte_rm, "F:/Projetos/mobilidados/datasus_verificacao/output/tab_morte_rm_negres.csv", row.names = FALSE)
