...
combined_data2 <- combined_data %>%
  ...
  summarise(total = sum(internacoes)) %>%
...
tx_mun <- result %>%
  mutate(tx_inter = total/valor*100000)

tx_mun <- tx_mun %>% select(-valor, -total) %>% pivot_wider(names_from = "ano", values_from = "tx_inter") 
...
rms_rlt <- tx_rms_mort %>% select(-total, -valor) %>% pivot_wider(names_from = "ano", values_from = "tx_inter")
