...
baixar_dados_raca <- function (i){
  ...
  tot_fim <- tot %>% mutate(code_muni6 = as.character(substr(Município, 1, 6)), tipo = "pedestres", 
                            !!paste0("negros_", i) := Preta+Parda, !!paste0("total_", i) := Total)
  # head(tot_fim)
  
  
  tot_final <- tot_fim %>% select(code_muni6, paste0("negros_", i), paste0("total_", i))
  # head(tot_final)
...
# aplicar função
baixar_dados_raca("2014")

pblapply(2015:2018, baixar_dados_raca)
