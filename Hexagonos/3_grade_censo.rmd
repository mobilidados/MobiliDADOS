...
munis_df2 <- data.frame(code_muni = c(3305109,2304400,3550308,4106902,3106200,
                                     2927408,1501402,5300108), 
                       name_muni=c('sao joao de meriti','fortaleza','sao paulo','curitiba',
                                   'belo horizonte','salvador','belem','distrito federal'),
                       abrev_state=c('RJ','CE','SP','PR','MG','BA','PA','DF'),
                       rm=c('rmrj','rmf','rmsp','rmc','rmbh','rms','rmb','ride-df'))
...
criar_grade_muni <- function(sigla){
  ...
  cod_estado <- subset(munis_df2, name_muni==sigla)$abrev_state %>% as.character()
  ...
  muni <- readr::read_rds(paste0("./dados/", subset(munis_df2, name_muni==sigla)$rm,
                                 "/municipios/municipio_", subset(munis_df2, name_muni==sigla)$name_muni,".rds"))
  ...
  # salvar no disco
  readr::write_rds(grade_muni, paste0("./dados/", subset(munis_df2, name_muni==sigla)$rm, 
                                      "/grade_municipio/grade_", sigla,".rds"))
}

# Aplicar funcao
lapply(X = munis_df2$name_muni, FUN=criar_grade_muni)
